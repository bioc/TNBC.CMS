...
#'@param gene.set Gene sets provided as a list. If NULL, the hallmark pathway gene sets are used.
#'@param gsva.kcdf Kernel to be used in the estimation of the cumulative distribution function. By default, this is set to \code{"Gaussian"} which is suitable for continuous expression values. If expression values are counts, \code{"Poisson"} is recommended.
#'@return A matrix of GSVA enrichment scores.
#'@details This is a wrapper function of the \code{gsva} function in the \code{GSVA} package to compute GSVA enrichment scores per sample and produce a heatmap comparing them across consensus molecular subtypes.
...
performGSVA <- function(exp.mat, pred, gene.set = NULL, gsva.kcdf = "Gaussian"){
  ...
  if(!is.null(gene.set)){
    GSVA.score <- gsva(as.matrix(exp.mat), gene.set, kcdf = "Gaussian", verbose = F)
  ...
  GSVA.score <- gsva(as.matrix(exp.mat), Hallmark.geneset, kcdf = "Gaussian", verbose = FALSE)
