...
includes a total of 12,722 genes and 73 samples. Note that rows and columns 
correspond to genes and samples, and row names must be gene symbols.
<<loadPackagesAndData>>=
library(TNBC.CMS)
data("GSE25055.exprs")
dim(GSE25055.exprs)
GSE25055.exprs[1:5, 1:5]
@
...
<<prediction>>=
predictions <- predictCMS(GSE25055.exprs)
table(predictions)
...
resultGES <- computeGES(exp.mat = GSE25055.exprs, pred = predictions, 
                        rnaseq = FALSE)
resultGES[,1:4]
@
...
time <- GSE25055.clinical$DFS.month
...
exprs <- GSE25055.exprs[apply(GSE25055.exprs, 1, var) > 0.5, ]
chisq <- apply(exprs, 1, function(x) survdiff(surv ~ (x > median(x)))$chisq)
pval <- 1 - pchisq(chisq, 1)

#Select 10 genes with lowest p-values for the log-rank test
gs <- names(sort(pval)[1:10])
gs
plotHR(exp.mat = GSE25055.exprs, gene.symbol = gs, pred = predictions, time = time,
...
plotHR(exp.mat = GSE25055.exprs, gene.symbol = gs[1:4], pred = predictions, 
